export function getPresetDefinitions() {
	...
	function addPPTPreset(i) {
		const id = i < 10 ? `0${i}` : `${i}`
		createPreset(`ppt${id}`, 'PPT', `Send PPT ${id} Command`, `PPT\n${id}`, '16777215', '139', `PPT${id}`)
	}
	...
	function addVideoPreset(i) {
		const id = i < 10 ? `0${i}` : `${i}`
		createPreset(`video${id}`, 'VIDEO', `Send VIDEO ${id} Command`, `VIDEO\n${id}`, '16777215', '25600', `VIDEO${id}`)
	}
